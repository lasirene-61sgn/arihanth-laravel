...
$content = preg_replace(
    '/cell\.textContent\s*=\s*day;\s*cell\.classList\.add\(\'tw-py-2\',\s*\'tw-cursor-pointer\',\s*\'tw-rounded-lg\',\s*\'hover:tw-bg-gray-100\',\s*\'dark:hover:tw-bg-slate-800\',\s*\'tw-transition-all\'\);/',
...
// 2. Fix showEventsForDate to handle the NEW aggregated JSON format
$newShowEvents = <<<'JS'
...
$content = preg_replace(
    '/function showEventsForDate\(dateStr\)\s*\{(?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*\}/s',
    $newShowEvents . "\n        }",
    $content
);

// 3. Fix calendar highlight logic
$content = preg_replace(
    '/\/\/\s*Highlight days with events or holidays\s*if\s*\(window\.calendarEvents\s*&&\s*window\.calendarEvents\[dateStr\]\)\s*\{(?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*\}/s',
