...
@section('content')
<style>
    body.chat-page-admin { overflow: hidden; }
    .chat-page-wrap { display:flex; height:calc(100vh - 70px); border-radius:12px; overflow:hidden; box-shadow:0 4px 24px rgba(13,110,253,0.1); border:1px solid #dee2e6; }
    .chat-sidebar { width:300px; min-width:260px; background:#fff; border-right:1px solid #dee2e6; display:flex; flex-direction:column; }
    .chat-sidebar-header { padding:14px 16px; background:#0d6efd; display:flex; align-items:center; justify-content:space-between; }
    ...
    .unread-badge { background:#0d6efd; color:#fff; font-size:0.65rem; font-weight:800; min-width:18px; height:18px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; padding:0 4px; }
    .chat-area { flex:1; display:flex; flex-direction:column; background:#f8f9fa; min-width:0; }
    .chat-area-header { padding:14px 20px; background:#fff; border-bottom:1px solid #dee2e6; display:flex; align-items:center; justify-content:space-between; }
    ...
    .attach-preview-item img { width:60px; height:60px; object-fit:cover; border-radius:8px; border:2px solid #bee2fd; }
    ...
    .msg-input:focus { border-color:#86b7fe; }
...
<div class="chat-page-wrap">
    ...
    <div class="chat-sidebar">
        <div class="chat-sidebar-header">
            <div>
                <div style="color:#fff;font-weight:800;font-size:1rem;">💬 Admin Chat</div>
                <small style="color:#cfe2ff;font-size:0.75rem;">{{ $user->name ?? 'Admin' }}</small>
            </div>
            <button type="button" class="attach-btn-sq" style="background:rgba(255,255,255,.2);color:#fff;" data-bs-toggle="modal" data-bs-target="#newAdminChatModal" title="New Message">+</button>
        </div>
        <div class="convo-list" id="admin-convo-list">
...
{{-- Modal --}}
<div class="modal fade" id="newAdminChatModal" tabindex="-1" aria-hidden="true">
                ...
                <div id="chat-search-results" style="max-height:350px;overflow-y:auto;">
                    @if(isset($suggestedContacts) && count($suggestedContacts) > 0)
                        @foreach($suggestedContacts as $contact)
                            ...
                            <a href="javascript:void(0)" onclick="startChatAjax('{{ route('admin.chat.start', ['receiverId'=>$contactObj->id??$contactObj->user_id]) }}','{{ addslashes($contactObj->name??'Contact') }}')"
                               class="list-group-item list-group-item-action p-3 d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <small class="text-muted">{{ $contactObj->code ?? '' }}</small>
                                </div>
...
<script>
    ...
    const searchRouteUrl   = "{{ route('admin.chat.search') }}";
    ...
    window.startChatAjax = function(url, titleName) {
        fetch(url, { headers: { 'X-Requested-With':'XMLHttpRequest','Accept':'application/json' } })
            .then(r => r.json())
            .then(data => {
                const modalEl = document.getElementById('newAdminChatModal');
    ...
    function moveConversationToTop(convoId,lastMsg) {
        const list=document.getElementById('admin-convo-list'); if(!list) return;
        ...
        fetch(`${searchRouteUrl}?q=${encodeURIComponent(query)}`).then(r=>r.json()).then(data=>{
            ...
            if(data.length>0){
                data.forEach(contact=>{
                    const url=`{{ url('admin/chat/start') }}/${contact.id}/${contact.type}`;
                    resultsBox.innerHTML+=`<a href="javascript:void(0)" onclick="startChatAjax('${url}','${contact.name.replace(/'/g,"\\'")}') "
                        ...
                        <span class="btn btn-sm btn-primary">Start Chat</span></a>`;
                });
