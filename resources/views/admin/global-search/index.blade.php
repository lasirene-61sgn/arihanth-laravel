...
<div class="max-w-5xl mx-auto py-8">
    ...
    <!-- Search Form -->
    <div class="mb-10">
        <form action="{{ route('admin.global-search') }}" method="GET" class="relative" id="searchForm" onsubmit="return false;">
            <div class="relative flex items-center w-full h-16 rounded-2xl bg-white dark:bg-slate-900 shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-magenta transition-all">
                ...
                <input 
                    ...
                    autofocus 
                />
                <a href="{{ route('admin.global-search') }}" id="clearBtn" class="h-full grid place-items-center w-16 text-gray-400 hover:text-magenta transition-colors mr-2 {{ request('search') ? '' : 'hidden' }}">
                        ...
                        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-gray-100 dark:border-slate-800 overflow-hidden">
                            ...
                            <ul class="divide-y divide-gray-50 dark:divide-slate-800/50 max-h-64 overflow-y-auto">
                                @foreach($data['items'] as $item)
                                    <li>
                                        <a href="{{ $item['url'] }}" class="flex items-center justify-between px-5 py-3 hover:bg-magenta/5 dark:hover:bg-slate-800 transition-colors group">
                                            <div class="flex items-center gap-3">
                                                <div class="w-8 h-8 rounded-full bg-magenta/10 flex items-center justify-center text-magenta">
                                                    <i class="bi bi-link-45deg"></i>
                                                </div>
                                                <span class="text-gray-700 dark:text-gray-300 font-medium group-hover:text-magenta transition-colors">{{ $item['display'] }}</span>
                                            </div>
                                            <i class="bi bi-chevron-right text-gray-300 group-hover:text-magenta transition-colors text-sm"></i>
                                        </a>
                                    </li>
                                @endforeach
                            </ul>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`{{ route('admin.global-search') }}?search=${encodeURIComponent(query)}`, {
                    headers: {
