...
<div class="max-w-5xl mx-auto py-8">
    ...
    <!-- Search Form -->
    <div class="mb-10">
        <form action="{{ route('admin.global-search') }}" method="GET" class="relative" id="searchForm" onsubmit="return false;" enctype="multipart/form-data">
            <div class="relative flex items-center w-full h-16 rounded-2xl bg-white dark:bg-slate-900 shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-magenta transition-all">
                ...
                <label for="imageUploadInput" class="h-full grid place-items-center w-14 text-gray-400 hover:text-magenta transition-colors cursor-pointer" title="Search by image">
                    <i class="bi bi-camera text-xl"></i>
                </label>
                <input type="file" id="imageUploadInput" class="hidden" accept="image/*" name="image_search" />

                <!-- Clear Button -->
                <a href="{{ route('admin.global-search') }}" id="clearBtn" class="h-full grid place-items-center w-14 text-gray-400 hover:text-magenta transition-colors mr-2 {{ request('search') ? '' : 'hidden' }}">
            ...
            <div id="imagePreviewContainer" class="hidden mt-4 flex items-center justify-between bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm max-w-md mx-auto animate-fade-in-up">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-magenta">Visual similarity matching</p>
                    </div>
                </div>
        ...
        @if(request()->has('search') && !empty($query))
            @if(empty($results))
                <div class="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 animate-fade-in-up">
                    ...
                    <p class="text-gray-500 dark:text-gray-400">We couldn't find anything matching "<span class="font-medium text-magenta">{{ $query }}</span>".</p>
                </div>
            @else
                <div class="mb-4 flex items-center justify-between animate-fade-in-up">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
                        Search Results for "<span class="text-magenta">{{ $query }}</span>"
                    </h3>
                                    ...
                                    <li>
                                        <button type="button" 
                                            ...
                                            <div class="flex items-center gap-3.5 min-w-0">
                                                ...
                                                @endif
                                                <div class="min-w-0">
                                                    <span class="text-gray-800 dark:text-gray-200 font-semibold group-hover:text-magenta transition-colors block truncate">{{ $item['display'] }}</span>
                                                    <p class="text-xs text-gray-400 dark:text-gray-500 truncate mt-0.5">{{ $item['details'] ?? 'No additional details available.' }}</p>
                                                </div>
                                            </div>
                                            <i class="bi bi-chevron-right text-gray-300 group-hover:text-magenta transition-colors text-sm mt-3 ml-2 shrink-0"></i>
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Function to render results preserving your existing design
        ...
        // Search execution handler (text or image)
        const performSearch = () => {
            ...
            searchIconContainer.innerHTML = `<div class="spinner-border text-magenta w-5 h-5" role="status"></div>`;

            clearTimeout(debounceTimer);
            debounceTimer = setTimeout(() => {
                const formData = new FormData();
                let url = `{{ route('admin.global-search') }}`;
                let options = {
                    headers: {
