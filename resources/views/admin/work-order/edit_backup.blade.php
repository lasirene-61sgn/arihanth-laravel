                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-control @error('product_category_id') is-invalid @enderror" 
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ (old('product_category_id') == $category->id || $workOrder->product_category == $category->name) ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                refreshSubcategories(categorySelect.value).then(() => {
                    @if($workOrder->subcategory)
                        ...
                        for (let i = 0; i < subcategoryOptions.length; i++) {
                            if (subcategoryOptions[i].text === '{{ $workOrder->subcategory }}') {
