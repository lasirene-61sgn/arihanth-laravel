...
<div class="p-4 md:p-6 lg:p-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight">Buyer Management</h1>
            <p class="text-gray-500 text-sm mt-1">Manage, filter and export your business buyers.</p>
        </div>

        <div class="flex flex-wrap items-center gap-3">
            <a href="{{ route('admin.business-partner.buyer.export', request()->all()) }}"
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('admin.business-partner.buyer') }}" id="filterForm">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <!-- State -->
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">State</label>
                        <select name="state" class="block w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-magenta-500 focus:border-magenta-500 text-sm transition-all shadow-sm">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
            <h4 class="font-bold text-gray-900">Buyer List</h4>
            <div class="flex items-center gap-2 text-xs font-bold text-gray-400">
                <i class="bi bi-info-circle"></i>
                <span>Showing {{ $buyers->count() }} results</span>
            </div>
        </div>
                ...
                <thead>
                    <tr class="text-[0.65rem] uppercase tracking-widest text-gray-400 border-b border-gray-50 bg-white">
                        ...
                        <th class="px-6 py-4 font-bold">
                            <a href="{{ route('admin.business-partner.buyer', array_merge(request()->query(), ['sort' => request('sort') == 'code_asc' ? 'latest' : 'code_asc'])) }}"
                                class="flex items-center gap-2 hover:text-magenta-600 transition-colors">
                                BP Code
                                @if(request('sort') == 'code_asc')
                                <i class="bi bi-arrow-up text-magenta-600"></i>
                                @elseif(request('sort') == 'latest' || !request('sort'))
                                <i class="bi bi-arrow-down text-magenta-600"></i>
                                @else
                        ...
                        <th class="px-6 py-4 font-bold">
                            <a href="{{ route('admin.business-partner.buyer', array_merge(request()->query(), ['sort' => request('sort') == 'name_asc' ? 'latest' : 'name_asc'])) }}"
                                class="flex items-center gap-2 hover:text-magenta-600 transition-colors">
                                Business Name
                        ...
                        <th class="px-6 py-4 font-bold">
                            <a href="{{ route('admin.business-partner.buyer', array_merge(request()->query(), ['sort_by' => 'name', 'sort_order' => request('sort_by') == 'name' && request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}"
                                class="flex items-center gap-2 hover:text-magenta-600 transition-colors">
                                Contact Person
                ...
                <tbody class="divide-y divide-gray-50 text-sm">
                    ...
                    <tr class="hover:bg-magenta-50/30 transition-colors duration-150 group">
                        <td class="px-6 py-4 text-center">
                            <input type="checkbox" class="row-checkbox w-4 h-4 text-magenta-600 border-gray-300 rounded focus:ring-magenta-500" value="{{ $buyer->id }}" name="selected_buyers[]">
                        </td>
                        <td class="px-6 py-4 font-bold text-magenta-600">{{ $buyer->bp_code }}</td>
                        <td class="px-6 py-4 font-semibold text-gray-900">{{ $buyer->business_name }}</td>
                        <td class="px-6 py-4 text-gray-600">{{ $buyer->name }}</td>
                        <td class="px-6 py-4 text-gray-600">{{ $buyer->mobile }}</td>
                        <td class="px-6 py-4 text-gray-500">{{ $buyer->email }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 bg-gray-100 text-gray-600 rounded text-[0.7rem] font-bold">{{ $buyer->city ?? 'N/A' }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-400 font-medium font-bold">No buyers found.</p>
                            </div>
                        </td>
                    </tr>
                    @endif
