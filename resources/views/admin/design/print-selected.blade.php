...
<head>
    ...
    <title>Selected Designs Report - Admin</title>
            ...
            <div class="design-card bg-white {{ !$loop->last ? 'page-break' : '' }}">
                ...
                <div class="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-8">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 font-medium">Production Specification Archive</p>
                    </div>
                    <div class="text-right">
                        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Entry Ref</div>
                        <div class="text-xl font-black text-gray-800 italic">#{{ $index + 1 }}</div>
                        ...
                        @if($design->design_status === 'Accepted' && $design->qr_code)
                            ...
                            @if($qrBase64)
                                <div class="mt-4 flex justify-end">
                                    <img src="{{ $qrBase64 }}" class="w-16 h-16 border border-slate-100 p-1">
                                </div>
                            @endif
                        @endif
                    </div>
                    ...
                    <div class="flex-1 space-y-8">
                        <div>
                            ...
                            <div class="flex gap-3">
                                ...
                                <span class="bg-slate-100 text-slate-600 px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest border border-slate-200">
                                    {{ $design->design_status ?? 'Pending' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-x-8 gap-y-4">
                            <div>
                                ...
                                <p class="text-sm font-bold text-slate-800">{{ optional($design->category)->name ?? '---' }}</p>
                            </div>
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Sub-Category</p>
                                <p class="text-sm font-bold text-slate-800">{{ optional($design->subcategory)->name ?? '---' }}</p>
                            </div>
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Weight Constraint</p>
                                <p class="text-sm font-bold text-slate-900">{{ $design->weight_from }}g - {{ $design->weight_to }}g</p>
                            </div>
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Creator</p>
                                <p class="text-sm font-bold text-slate-700">{{ $design->creator_name }}</p>
                            </div>
                        </div>

                        <div class="bg-slate-50 rounded-xl p-6 border border-slate-100">
                            <h3 class="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-4 italic underline decoration-slate-200">Technical Brief</h3>
                        ...
                        @if($design->details)
                        <div class="border-l-2 border-slate-200 pl-4 py-1 font-serif italic text-slate-500 text-sm leading-relaxed">
                            {{ $design->details }}
                        </div>
                        @endif
                    </div>
                        ...
                        <div class="aspect-[3/4] w-full bg-slate-50 rounded-2xl border-2 border-slate-100 flex items-center justify-center p-3 shadow-inner relative overflow-hidden">
                            ...
                            @else
                                <div class="text-center">
                                    <div class="text-slate-200 text-5xl mb-2">📐</div>
                                    <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Image Pending</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="border-t-2 border-slate-100 pt-6 flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-12">
                    <div class="flex items-center gap-2">
                        <span class="bg-slate-900 text-white px-2 py-0.5 rounded text-[8px]">OFFICIAL</span>
                        <span>Arihanth Jewellers Digital Archive</span>
                    </div>
