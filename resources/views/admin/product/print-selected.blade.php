...
<head>
    ...
    <title>Selected Products Report - Admin</title>
            ...
            <div class="product-card bg-white {{ !$loop->last ? 'page-break' : '' }}">
                ...
                <div class="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-8">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 font-medium">Inventory Management Record</p>
                    </div>
                ...
                <div class="flex gap-10 items-start flex-grow">
                    ...
                    <div class="flex-1 space-y-8">
                        <div>
                            ...
                            <div class="flex gap-3">
                                ...
                                <span class="bg-slate-100 text-slate-500 px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest border border-slate-200 italic">
                        ...
                        <div class="grid grid-cols-2 gap-x-8 gap-y-4">
                            <div>
                                ...
                                <p class="text-sm font-bold text-slate-800 uppercase">{{ optional($product->category)->name ?? '---' }}</p>
                            </div>
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Sub-Category</p>
                                <p class="text-sm font-bold text-slate-800 uppercase">{{ optional($product->subcategory)->name ?? '---' }}</p>
                            </div>
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Weight Range</p>
                                <p class="text-sm font-black text-slate-900 underline underline-offset-4 decoration-slate-200">{{ $product->weight_from }}g - {{ $product->weight_to }}g</p>
                            </div>
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Quality Standards</p>
                                <div class="flex gap-1 mt-1">
                                    <span class="bg-slate-50 text-[10px] font-bold px-1.5 py-0.5 rounded border border-slate-200">{{ $product->hallmark }}</span>
                                    <span class="bg-slate-50 text-[10px] font-bold px-1.5 py-0.5 rounded border border-slate-200">{{ $product->rodium }}</span>
                                </div>
                            </div>
                        </div>

                        <div class="bg-slate-50 rounded-xl p-6 border border-slate-100 shadow-sm">
                            <h3 class="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-4 italic underline decoration-slate-200">Production DNA</h3>
                            <div class="grid grid-cols-3 gap-y-4">
                                ...
                                @foreach($techSpecs as $label => $val)
                                    <div class="flex flex-col">
                                        <span class="text-[9px] font-bold text-slate-400 uppercase">{{ $label }}</span>
                                        <span class="text-xs font-black text-slate-800">{{ $val ?: '---' }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="border-l-4 border-slate-900 pl-6 py-2">
                            <p class="text-[9px] font-black text-slate-400 uppercase mb-2">Item Narrative</p>
                    ...
                    <div class="w-80 shrink-0">
                        ...
                        <div class="aspect-[1/1] w-full bg-slate-50 rounded-2xl border-2 border-slate-100 flex items-center justify-center p-4 shadow-inner overflow-hidden">
                            @if($mainImage)
                            ...
                            @else
                                <div class="text-center">
                                    ...
                                    <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Metadata Only</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="border-t-2 border-slate-100 pt-6 flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-12">
                    <div class="flex items-center gap-2">
                        ...
                        <span>Arihanth Jewellers • Master Inventory</span>
                    </div>
                    <span>Record UID: {{ $product->id }}</span>
                    <span>System Print: {{ now()->format('d/m/Y | H:i') }}</span>
                </div>
            </div>
        @endforeach
    </div>
