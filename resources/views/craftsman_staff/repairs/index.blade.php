                            ...
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-inline-flex gap-1 align-items-center">
                                        <a href="{{ route('craftsman_staff.repairs.show', $repair->id) }}" class="btn btn-action btn-outline-secondary" title="View Details">
                                        ...
                                        @if($repair->craftsman_status == 'Pending')
                                            ...
                                            @if(!$staffUser || $staffUser->hasPermission('repair_accept'))
                                            <form action="{{ route('craftsman_staff.repairs.accept', $repair->id) }}" method="POST" class="d-inline">
                                                @csrf
                                            ...
                                            {{-- Reject --}}
                                            @php $staffUser = auth()->guard('craftsman_staff')->user(); @endphp
                                            @if(!$staffUser || $staffUser->hasPermission('repair_reject'))
                                            <form action="{{ route('craftsman_staff.repairs.reject', $repair->id) }}" method="POST" class="d-inline">
                                                @csrf
                                        ...
                                        @elseif($repair->craftsman_status == 'Accepted')
                                            ...
                                            @php $staffUser = auth()->guard('craftsman_staff')->user(); @endphp
                                            @if(!$staffUser || $staffUser->hasPermission('repair_accept'))
                                            <form action="{{ route('craftsman_staff.repairs.complete', $repair->id) }}" method="POST" class="d-inline">
                                                @csrf
