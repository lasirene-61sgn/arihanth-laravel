...
<div class="space-y-6">
    <!-- Header -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-indigo-100 pb-5">
        <div>
            <h1 class="text-2xl font-bold text-indigo-900 tracking-tight">My Work Orders</h1>
            @if(optional(Auth::guard('craftsman')->user())->dear)
                <p class="text-indigo-600 mt-1">Welcome back, <span class="font-semibold text-indigo-800">{{ Auth::guard('craftsman')->user()->dear }}</span></p>
            @elseif(optional(Auth::guard('craftsman_staff')->user())->name)
                <p class="text-indigo-600 mt-1">Welcome back, <span class="font-semibold text-indigo-800">{{ Auth::guard('craftsman_staff')->user()->name }}</span></p>
            @endif
        </div>
    </div>
    ...
    <!-- Notifications -->
    @if (session('success'))
        <div class="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded shadow-sm flex justify-between items-center transition-all duration-300">
            <div class="flex items-center gap-3">
                <i class="bi bi-check-circle-fill text-indigo-500 text-xl"></i>
                <p class="text-indigo-800 font-medium">{{ session('success') }}</p>
            </div>
            <button type="button" class="text-indigo-400 hover:text-indigo-600" onclick="this.parentElement.remove()">
    ...
    <div class="bg-white p-6 rounded-2xl border border-indigo-100 shadow-sm">
        <form action="{{ route('craftsman_staff.work-order.index') }}" method="GET" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 items-end">
                <div class="lg:col-span-3">
                    <label class="block text-xs font-bold uppercase tracking-wider text-indigo-600 mb-2">Search Work Orders</label>
                    <div class="relative group">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-indigo-400 group-focus-within:text-indigo-600 transition-colors">
                            <i class="bi bi-search"></i>
                        </span>
                        <input type="text" name="search" 
                               class="w-full pl-10 pr-4 py-2 bg-indigo-50 border border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" 
                ...
                <div class="lg:col-span-3">
                    <label class="block text-xs font-bold uppercase tracking-wider text-indigo-600 mb-2">Category</label>
                    <div id="category_filter_container" class="relative">
                        <div id="category_filter_display" class="w-full pl-4 pr-10 py-2 bg-indigo-50 border border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none cursor-pointer flex items-center justify-between text-sm">
                            <span class="truncate">All Categories</span>
                            <i class="bi bi-chevron-down text-indigo-400 text-[10px]"></i>
                        </div>
                        <div id="category_filter_menu" class="absolute z-50 w-full mt-2 bg-white border border-indigo-100 rounded-xl shadow-xl hidden overflow-hidden">
                            <div class="p-2 border-b border-indigo-50 bg-indigo-50/30">
                                <input type="text" id="category_filter_search" class="w-full px-3 py-1.5 text-xs bg-white border border-indigo-100 rounded-lg focus:outline-none focus:border-indigo-500" placeholder="Search categories...">
                            </div>
                            <ul id="category_filter_list" class="max-h-60 overflow-y-auto no-scrollbar py-1">
                                <li class="px-4 py-2 text-sm text-indigo-600 hover:bg-indigo-50 cursor-pointer transition-colors font-medium border-b border-indigo-50/50" data-value="">
                                    All Categories
                                </li>
                                @foreach($productCategories as $cat)
                                    <li class="px-4 py-2 text-sm text-indigo-900 hover:bg-indigo-50 cursor-pointer transition-colors" data-value="{{ $cat->id }}">
                                        {{ $cat->name }}
                                    </li>
                                @endforeach
                            </ul>
                ...
                <div class="lg:col-span-2">
                    <label class="block text-xs font-bold uppercase tracking-wider text-indigo-600 mb-2">Sort By</label>
                    <select name="sort_by" class="w-full px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all appearance-none cursor-pointer text-sm">
                ...
                <div class="lg:col-span-1">
                    <label class="block text-xs font-bold uppercase tracking-wider text-indigo-600 mb-2">Order</label>
                    <select name="sort_order" class="w-full px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all appearance-none cursor-pointer text-sm">
                ...
                <div class="lg:col-span-1">
                    <label class="block text-xs font-bold uppercase tracking-wider text-indigo-600 mb-2">Size</label>
                    <select name="per_page" class="w-full px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all appearance-none cursor-pointer text-sm">
                ...
                <div class="lg:col-span-2 flex gap-2">
                    <button type="submit" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-3 rounded-xl transition-all shadow-sm flex items-center justify-center gap-1.5 text-xs">
                        <i class="bi bi-filter"></i> Apply
                    </button>
                    <a href="{{ route('craftsman_staff.work-order.index') }}" class="flex-1 bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-bold py-2 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 text-xs">
            ...
            <div class="pt-4 border-t border-indigo-100 flex justify-end">
                <a href="{{ route('craftsman_staff.work-order.export', request()->all()) }}" 
                   class="bg-indigo-100 text-indigo-700 hover:bg-indigo-200 font-bold py-2 px-6 rounded-xl transition-all flex items-center justify-center gap-2 text-sm">
    ...
    <!-- Navigation Tabs -->
    <div class="border-b border-indigo-100 mb-4">
    ...
    <style>
        ...
        #workOrderTabs button:hover {
            color: #047857; /* indigo-700 */
        }
        
        #workOrderTabs button.active {
            border-bottom-color: #059669; /* indigo-600 */
            color: #064e3b; /* indigo-900 */
            background-color: rgba(236, 253, 245, 0.5); /* indigo-50/50 */
    ...
    <div class="tab-content" id="workOrderTabsContent">
            ...
            <div class="tab-pane fade {{ request('tab', 'allocated') == 'allocated' ? 'show active' : '' }}" id="allocated" role="tabpanel">
                <div class="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden mt-4">
                    <div class="p-4 bg-indigo-50/50 border-b border-indigo-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div class="flex items-center gap-3">
                            <h4 class="font-bold text-indigo-900">Allocated Work Orders</h4>
                            <span class="bg-indigo-600 text-white text-xs font-bold px-2 py-1 rounded-full">{{ $allocatedOrders->total() }}</span>
                        </div>
                    </div>

                    <div class="p-0">
                        @if($allocatedOrders->count() > 0)
                            <form action="{{ route('craftsman_staff.work-order.bulk-accept') }}" method="POST" id="bulkAcceptForm">
                                @csrf
                                <div class="px-6 py-4 bg-white border-b border-indigo-50 flex flex-wrap justify-between items-center gap-3">
                                    <div class="flex items-center gap-2">
                                        @php $staffUser = auth()->guard('craftsman_staff')->user(); @endphp
                                        @if(!$staffUser || $staffUser->hasPermission('wo_accept'))
                                        <button type="button" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold py-2 px-4 rounded-xl transition-all shadow-sm flex items-center gap-2" 
                                                onclick="if(validateBulkSelection()) document.getElementById('bulkAcceptForm').submit();">
                                            <i class="bi bi-check-all"></i> Bulk Accept
                                        </button>
                                        @endif
                                        @if(!$staffUser || $staffUser->hasPermission('wo_reject'))
                                    ...
                                    <div>
                                        <button type="submit" formaction="{{ route('craftsman_staff.work-order.print-selected') }}" formmethod="POST" 
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full text-left">
                                        <thead>
                                            <tr class="bg-indigo-50/30 text-indigo-800 text-xs font-bold uppercase tracking-wider">
                                                <th class="px-6 py-4 text-center"><input type="checkbox" id="selectAllAllocated" class="rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500"></th>
...
@endphp
                                <tr class="hover:tw-bg-gray-50 tw-transition-colors  hover:bg-indigo-50/30 transition-colors {{ ($order->priority == 'Urgent' || $order->priority == 'High') && $order->craftsman_status === 'allocated' ? 'animate-custom-blink' : '' }}" style="{{ $rowStyle }}">
                                                    <td class="px-6 py-4 text-center">
                                                        <input type="checkbox" name="work_order_ids[]" value="{{ $order->id }}" class="allocated-checkbox rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500">
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        @if($order->preview_image_url)
                                                            <div class="w-12 h-12 rounded-lg border border-indigo-100 overflow-hidden cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all bg-white"
                                                        ...
                                                        @else
                                                            <div class="w-12 h-12 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-300">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="flex flex-col gap-1">
                                                            <span class="font-bold text-indigo-900">{{ $order->work_order_number }}</span>
                                                            @if($order->priority)
                                                    ...
                                                    <td class="px-6 py-4 text-sm text-indigo-700">{{ $order->productCategory->name ?? (is_array($order->product_category) ? implode(', ', $order->product_category) : ($order->product_category ?? '-')) }}</td>
                                                    <td class="px-6 py-4 text-sm text-indigo-700">{{ $order->subcategoryRelation->name ?? (is_array($order->subcategory) ? implode(', ', $order->subcategory) : ($order->subcategory ?? '-')) }}</td>
                                                    ...
                                                    <td class="px-6 py-4 text-right">
                                                        <a href="{{ route('craftsman_staff.work-order.show', $order) }}" 
                                                           class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-sm" title="View Details">
                                                            <i class="bi bi-eye"></i>
                                                        </a>
                                                    </td>
                                                </tr>
                            ...
                            <div class="p-6 border-t border-indigo-50">
                                {{ $allocatedOrders->appends(array_merge(request()->except('allocated_orders_page'), ['tab' => 'allocated']))->links() }}
                            </div>
                        @else
                            <div class="text-center py-12">
                                <div class="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <i class="bi bi-inbox text-2xl text-indigo-300"></i>
                                </div>
                                <h3 class="text-indigo-900 font-bold">No Allocated Orders</h3>
                                <p class="text-indigo-500 text-sm">You have no new work orders allocated to you.</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="tab-pane fade {{ request('tab') == 'in-process' ? 'show active' : '' }}" id="in-process" role="tabpanel">
                    <div class="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden mt-4">
                        <div class="p-4 bg-indigo-50/50 border-b border-indigo-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div class="flex items-center gap-3">
                                <h4 class="font-bold text-indigo-900">In Process Work Orders</h4>
                        ...
                        <div class="p-0">
                            @if($inProcessOrders->count() > 0)
                                <form action="{{ route('craftsman_staff.work-order.bulk-complete') }}" method="POST" id="bulkCompleteForm">
                                    @csrf
                                    <div class="px-6 py-4 bg-white border-b border-indigo-50 flex flex-wrap justify-between items-center gap-3">
                                        <div>
                                            @php $staffUser = auth()->guard('craftsman_staff')->user(); @endphp
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-left">
                                            <thead>
                                                <tr class="bg-indigo-50/30 text-indigo-800 text-xs font-bold uppercase tracking-wider">
                                                    <th class="px-6 py-4 text-center"><input type="checkbox" id="selectAllInProcess" class="rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500"></th>
                                                    <th class="px-6 py-4">Image</th>
                                                    <th class="px-6 py-4">Work Order</th>
                                                    <th class="px-6 py-4">Category</th>
                                                    <th class="px-6 py-4">Subcategory</th>
...
@endphp
                                <tr class="hover:tw-bg-gray-50 tw-transition-colors  hover:bg-indigo-50/30 transition-colors {{ ($order->priority == 'Urgent' || $order->priority == 'High') && $order->craftsman_status === 'allocated' ? 'animate-custom-blink' : '' }}" style="{{ $rowStyle }}">
                                                        <td class="px-6 py-4 text-center">
                                                            <input type="checkbox" name="work_order_ids[]" value="{{ $order->id }}" class="in-process-checkbox rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500">
                                                        </td>
                                                        <td class="px-6 py-4">
                                                            @if($order->preview_image_url)
                                                                <div class="w-12 h-12 rounded-lg border border-indigo-100 overflow-hidden cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all bg-white"
                                                        ...
                                                        <td class="px-6 py-4 text-sm text-indigo-700">{{ $order->productCategory->name ?? $order->product_category ?? '-' }}</td>
                                                        <td class="px-6 py-4 text-sm text-indigo-700">{{ $order->subcategoryRelation->name ?? $order->subcategory ?? '-' }}</td>
                                                        <!--<td class="px-6 py-4 text-sm text-indigo-700 whitespace-nowrap">{{ $order->due_date ? $order->due_date->format('d M, Y') : 'N/A' }}</td>-->
                                                        <td class="px-6 py-4 text-sm {{ $isOverdue ? 'text-red-600 font-bold' : 'text-indigo-700' }} whitespace-nowrap">{{ $order->craftsman_due_date ? $order->craftsman_due_date->format('d M, Y') : 'N/A' }}</td>
                                                        <td class="px-6 py-4 text-sm text-indigo-700">{{ $order->weight_from }}g</td>
                                                        <td class="px-6 py-4 text-sm text-indigo-700 font-bold">{{ $order->quantity }}</td>
                                                        ...
                                                        <td class="px-6 py-4 text-right">
                                                            <div class="flex justify-end gap-2">
                                                                ...
                                                                <button type="button" onclick="showSingleCompleteModal('{{ route('craftsman_staff.work-order.complete', $order) }}')" 
                                                                        class="inline-flex items-center justify-center h-8 px-3 rounded-lg bg-indigo-600 text-white text-xs font-bold hover:bg-indigo-700 transition-all shadow-sm gap-2" title="Mark as Completed">
                            ...
                            @else
                                <div class="text-center py-12">
                                    <div class="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <i class="bi bi-play-circle text-2xl text-indigo-300"></i>
                                    </div>
                                    <h3 class="text-indigo-900 font-bold">No In-Process Orders</h3>
                                    <p class="text-indigo-500 text-sm">You have no work orders currently in process.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="tab-pane fade {{ request('tab') == 'completed' ? 'show active' : '' }}" id="completed" role="tabpanel">
                    <div class="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden mt-4">
                        <div class="p-4 bg-indigo-50/50 border-b border-indigo-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div class="flex items-center gap-3">
                                <h4 class="font-bold text-indigo-900">Completed Work Orders</h4>
                                <span class="bg-indigo-600 text-white text-xs font-bold px-2 py-1 rounded-full">{{ $completedOrders->total() }}</span>
                            </div>
                        </div>

                        <div class="p-0">
                            @if($completedOrders->count() > 0)
                                <form action="{{ route('craftsman_staff.work-order.print-selected') }}" method="POST" id="bulkPrintCompletedForm">
                                    @csrf
                                    <div class="px-6 py-4 bg-white border-b border-indigo-50 flex justify-end">
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-left">
                                            <thead>
                                                <tr class="bg-indigo-50/30 text-indigo-800 text-xs font-bold uppercase tracking-wider">
                                                    <th class="px-6 py-4 text-center"><input type="checkbox" id="selectAllCompleted" class="rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500"></th>
                                                    <th class="px-6 py-4">Image</th>
                                                    <th class="px-6 py-4">Work Order</th>
                                                    <th class="px-6 py-4">Category</th>
                                                    <th class="px-6 py-4">Subcategory</th>
                                            ...
                                            <tbody class="divide-y divide-indigo-50">
                                                @foreach($completedOrders as $order)
                                                    <tr class="hover:bg-indigo-50/30 transition-colors {{ ($order->priority == 'Urgent' || $order->priority == 'High') && $order->craftsman_status === 'allocated' ? 'animate-custom-blink' : '' }}">
                                                        <td class="px-6 py-4 text-center">
                                                            <input type="checkbox" name="work_order_ids[]" value="{{ $order->id }}" class="completed-checkbox rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500">
                                                        </td>
                                                        <td class="px-6 py-4">
                                                            @if($order->preview_image_url)
                                                                <div class="w-12 h-12 rounded-lg border border-indigo-100 overflow-hidden cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all bg-white"
                                                        ...
                                                        <td class="px-6 py-4">
                                                            ...
                                                            @elseif($order->status == 'completed')
                                                                <span class="px-2 py-1 bg-indigo-100 text-indigo-700 text-[10px] font-bold uppercase tracking-wider rounded-md">Approved</span>
                                                            @endif
                                                        </td>
                                                        <td class="px-6 py-4 text-right">
                                                            <a href="{{ route('craftsman_staff.work-order.show', $order) }}" 
                            ...
                            @else
                                <div class="text-center py-12">
                                    <div class="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <i class="bi bi-check-all text-2xl text-indigo-300"></i>
                                    </div>
                                    <h3 class="text-indigo-900 font-bold">No Completed Orders</h3>
                                    <p class="text-indigo-500 text-sm">You have no completed work orders yet.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="tab-pane fade {{ request('tab') == 'rejected' ? 'show active' : '' }}" id="rejected" role="tabpanel">
                    <div class="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden mt-4">
                        <div class="p-4 bg-indigo-50/50 border-b border-indigo-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div class="flex items-center gap-3">
                                <h4 class="font-bold text-indigo-900">Rejected Work Orders</h4>
                        ...
                        <div class="p-0">
                            @if($rejectedOrders->count() > 0)
                                <form action="{{ route('craftsman_staff.work-order.print-selected') }}" method="POST" id="bulkPrintRejectedForm">
                                    @csrf
                                    <div class="px-6 py-4 bg-white border-b border-indigo-50 flex justify-end">
                                        <button type="submit" class="bg-blue-50 text-blue-600 hover:bg-blue-100 text-sm font-bold py-2 px-4 rounded-xl transition-all flex items-center gap-2">
                                            <i class="bi bi-printer"></i> Print Selected
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-left">
                                            <thead>
                                                <tr class="bg-indigo-50/30 text-indigo-800 text-xs font-bold uppercase tracking-wider">
                                                    <th class="px-6 py-4 text-center"><input type="checkbox" id="selectAllRejected" class="rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500"></th>
                                                    <th class="px-6 py-4">Image</th>
                                                    <th class="px-6 py-4">Work Order</th>
                                                    <th class="px-6 py-4">Category</th>
                                                    <th class="px-6 py-4">Subcategory</th>
                                            ...
                                            <tbody class="divide-y divide-indigo-50">
                                                @foreach($rejectedOrders as $order)
                                                    <tr class="hover:bg-indigo-50/30 transition-colors {{ ($order->priority == 'Urgent' || $order->priority == 'High') && $order->craftsman_status === 'allocated' ? 'animate-custom-blink' : '' }}">
                                                        <td class="px-6 py-4 text-center">
                                                            <input type="checkbox" name="work_order_ids[]" value="{{ $order->id }}" class="rejected-checkbox rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500">
                                                        </td>
                                                        <td class="px-6 py-4">
                                                            @if($order->preview_image_url)
                                                                <div class="w-12 h-12 rounded-lg border border-indigo-100 overflow-hidden cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all bg-white"
                            ...
                            @else
                                <div class="text-center py-12">
                                    <div class="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <i class="bi bi-x-circle text-2xl text-indigo-300"></i>
                                    </div>
                                    <h3 class="text-indigo-900 font-bold">No Rejected Orders</h3>
                                    <p class="text-indigo-500 text-sm">You have no rejected work orders.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="tab-pane fade {{ request('tab') == 'overdue' ? 'show active' : '' }}" id="overdue" role="tabpanel">
                    <div class="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden mt-4">
                        <div class="p-4 bg-indigo-50/50 border-b border-indigo-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div class="flex items-center gap-3">
                                <h4 class="font-bold text-indigo-900">Overdue Work Orders</h4>
                        ...
                        <div class="p-0">
                            @if($overdueOrders->count() > 0)
                                <form action="{{ route('craftsman_staff.work-order.print-selected') }}" method="POST" id="bulkPrintOverdueForm">
                                    @csrf
                                    <div class="px-6 py-4 bg-white border-b border-indigo-50 flex justify-end">
                                        <button type="submit" class="bg-blue-50 text-blue-600 hover:bg-blue-100 text-sm font-bold py-2 px-4 rounded-xl transition-all flex items-center gap-2">
                                            <i class="bi bi-printer"></i> Print Selected
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-left">
                                            <thead>
                                                <tr class="bg-indigo-50/30 text-indigo-800 text-xs font-bold uppercase tracking-wider">
                                                    <th class="px-6 py-4 text-center"><input type="checkbox" id="selectAllOverdue" class="rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500"></th>
                                                    <th class="px-6 py-4">Image</th>
                                                    <th class="px-6 py-4">Work Order</th>
                                                    <th class="px-6 py-4">Category</th>
                                                    <th class="px-6 py-4">Subcategory</th>
                                            ...
                                            <tbody class="divide-y divide-indigo-50">
                                                @foreach($overdueOrders as $order)
                                                    <tr class="hover:bg-indigo-50/30 transition-colors bg-red-50/30 {{ ($order->priority == 'Urgent' || $order->priority == 'High') && $order->craftsman_status === 'allocated' ? 'animate-custom-blink' : '' }}">
                                                        <td class="px-6 py-4 text-center">
                                                            <input type="checkbox" name="work_order_ids[]" value="{{ $order->id }}" class="overdue-checkbox rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500">
                                                        </td>
                                                        <td class="px-6 py-4">
                                                            @if($order->preview_image_url)
                                                                <div class="w-12 h-12 rounded-lg border border-indigo-100 overflow-hidden cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all bg-white"
                            ...
                            @else
                                <div class="text-center py-12">
                                    <div class="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <i class="bi bi-calendar-check text-2xl text-indigo-300"></i>
                                    </div>
                                    <h3 class="text-indigo-900 font-bold">No Overdue Orders</h3>
                                    <p class="text-indigo-500 text-sm">Great job! You have no overdue work orders.</p>
                                </div>
                            @endif
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content !rounded-2xl border-none shadow-2xl overflow-hidden">
                <form id="completeOrderForm" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header bg-indigo-600 text-white border-none py-4 px-6">
                        <h5 class="modal-title font-bold text-lg">Complete Work Order</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body p-6 space-y-4">
                        <p id="completeModalMessage" class="text-indigo-800 font-medium">Upload images to document your completed work (optional):</p>
                        
                        <div class="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                            <label for="completion_images" class="block text-xs font-bold uppercase tracking-wider text-indigo-600 mb-2">Upload Images</label>
                            <input type="file" class="w-full text-sm text-indigo-700 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer" 
                                   name="images[]" id="completion_images" multiple accept="image/*">
                            <p class="text-[10px] text-indigo-500 mt-2 italic">Supported: JPG, PNG, WEBP. Max 5MB per image.</p>
                        </div>
                    ...
                    <div class="modal-footer bg-gray-50 p-4 border-t border-gray-100 flex gap-2">
                        ...
                        <button type="submit" class="flex-1 py-2 px-4 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all shadow-md">Confirm Completion</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    function initSearchableDropdown(containerId, displayId, menuId, searchInputId, listId, hiddenSelectId, placeholder) {
        ...
        listContainer.addEventListener('click', function(e) {
            ...
            getListItems().forEach(i => i.classList.remove('bg-indigo-50', 'font-bold'));
            item.classList.add('bg-indigo-50', 'font-bold');
        ...
        if (hiddenSelect.value) {
            ...
            if (selectedItem) {
                ...
                selectedItem.classList.add('bg-indigo-50', 'font-bold');
            }
        }
    }
</script>
<script>
    ...
    window.showBulkRejectModal = function() {
        ...
        rejectReasonForm.action = "{{ route('craftsman_staff.work-order.bulk-reject') }}";
    ...
    window.showBulkCompleteModal = function() {
        ...
        completeOrderForm.action = "{{ route('craftsman_staff.work-order.bulk-complete') }}";
