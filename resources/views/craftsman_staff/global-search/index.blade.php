...
<div class="max-w-5xl mx-auto py-8">
    <div class="mb-8 text-center">
        ...
        <p class="text-gray-600 dark:text-gray-400">Search across assigned Work Orders, Repairs, Purchase Orders, Products, and Stock Orders.</p>
    </div>

    <!-- Search Form -->
    <div class="mb-10">
        <form action="{{ route('craftsman_staff.global-search') }}" method="GET" class="relative" id="searchForm" onsubmit="return false;" enctype="multipart/form-data">
            <div class="relative flex items-center w-full h-16 rounded-2xl bg-white dark:bg-slate-900 shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-indigo-600 transition-all">
                ...
                <!-- Clear Button -->
                <a href="{{ route('craftsman_staff.global-search') }}" id="clearBtn" class="h-full grid place-items-center w-14 text-gray-400 hover:text-indigo-600 transition-colors mr-2 {{ request('search') ? '' : 'hidden' }}">
            ...
            <div id="imagePreviewContainer" class="hidden mt-4 flex items-center justify-between bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm max-w-md mx-auto animate-fade-in-up">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-indigo-600">Visual similarity matching</p>
                    </div>
                </div>
                                    ...
                                    <li>
                                        <button type="button" 
                                            ...
                                            <div class="flex items-center gap-3.5 min-w-0">
                                                ...
                                                @else
                                                    <div class="w-12 h-12 rounded-xl bg-indigo-600/10 flex items-center justify-center text-indigo-600 shrink-0">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderResults(query, results) {
            ...
            categories.forEach((category, index) => {
                ...
                data.items.forEach(item => {
                    ...
                    html += `
                        <li>
                            <button type="button" class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-indigo-600/5 dark:hover:bg-slate-800 transition-colors group"
                                data-url="${item.url}"
        ...
        const performSearch = () => {
            ...
            debounceTimer = setTimeout(() => {
                ...
                let url = `{{ route('craftsman_staff.global-search') }}`;
                let options = {
                    headers: {
