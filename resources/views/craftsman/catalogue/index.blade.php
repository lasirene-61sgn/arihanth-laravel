...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-emerald-200 pb-4">
        <div>
            <h4 class="text-2xl font-bold text-emerald-900">My Approved Catalogue</h4>
            <p class="text-sm text-emerald-600">Overview of your personalized approved designs</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('craftsman.catalogue.export', request()->query()) }}"
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-emerald-100 flex flex-wrap items-center gap-4">
        <form action="{{ route('craftsman.catalogue.index') }}" method="GET" class="relative flex-grow max-w-sm">
            ...
            <div class="dropdown-menu p-6 shadow-2xl border-0 rounded-xl" style="min-width: 400px;">
                <form action="{{ route('craftsman.catalogue.index') }}" method="GET" class="space-y-4">
                    ...
                    <div class="flex gap-2 pt-4 border-t border-emerald-50">
                        ...
                        <a href="{{ route('craftsman.catalogue.index') }}" class="flex-1 text-center border border-emerald-200 py-2 rounded-lg text-sm font-bold text-emerald-700 hover:bg-emerald-50 transition">Reset</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="group bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">

                <div class="relative h-56 bg-white overflow-hidden flex items-center justify-center p-4">
                    <div class="absolute top-3 left-3 z-10">
                        <input type="checkbox" name="selected_products[]" value="{{ $item->id }}" class="product-checkbox w-5 h-5 text-emerald-600 border-emerald-300 rounded focus:ring-emerald-500 shadow-sm transition-transform hover:scale-110 cursor-pointer">
                    </div>
                    @php
                ...
                <div class="p-4 flex flex-col flex-grow bg-white border-t border-emerald-50">
                    <div class="flex justify-between items-start mb-2">
                        <h6 class="font-bold text-emerald-900 truncate pr-2 w-2/3" title="{{ $item->product_name }}">
                            {{ $item->product_name }}
                        </h6>
                    ...
                    <div class="flex justify-between items-center text-sm mb-4">
                        <div class="text-xs">
                            <span class="text-emerald-400 font-bold uppercase tracking-tighter">Code:</span>
                    ...
                    <div class="mt-auto">
                        <a href="{{ route('craftsman.catalogue.show', $item->id) }}"
                            class="block w-full text-center bg-emerald-900 text-white text-sm font-bold py-2.5 rounded-xl hover:bg-emerald-800 transition-all active:scale-95 shadow-sm">
                            View Design
                        </a>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-emerald-100 p-20 text-center shadow-sm">
            <div class="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-4">
                <i class="bi bi-patch-question text-emerald-200 text-4xl"></i>
            </div>
            <h5 class="text-emerald-900 font-bold">No personal designs found.</h5>
            <p class="text-emerald-500 text-sm mt-1">Your approved catalogue is currently empty.</p>
        </div>
        @endif
    </div>
</div>

<form id="print_selected_form" action="{{ route('craftsman.catalogue.print-selected') }}" method="POST" target="_blank" class="hidden">
    @csrf
