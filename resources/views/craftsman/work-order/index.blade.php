                            ...
                            <table class="w-full text-left">
                                <thead>
                                    <tr class="bg-emerald-50/30 text-emerald-800 text-xs font-bold uppercase tracking-wider">
                                        ...
                                        <th class="px-6 py-4">Return Notes & Due Date</th>
                                    ...
                                    if ($hoursDiff >= 0 && $hoursDiff <= 48) {
                                        ...
                                        if ($hoursSinceUpdate <= 12) {
                                            ...
                                            if ($hoursSinceUpdate <= 12) {
                                                ...
                                                <td class="px-6 py-4 text-sm text-emerald-700 font-bold">{{ $order->quantity }} {{$order->type}}</td>
                                                ...
                                                <td class="px-6 py-4 text-sm">
                                                    <div class="flex flex-col gap-1">
                                                        {{-- Display Return Note if available --}}
                                                        @if($order->return_note)
                                                        <span class="text-xs text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded italic truncate max-w-[180px]"
                                                            title="{{ $order->return_note }}">
                                                            <strong class="not-italic uppercase font-bold text-[10px]">Note:</strong> "{{ $order->return_note }}"
                                                        </span>
                                                        @endif

                                                        {{-- Display Return Due Date if available --}}
                                                        @if($order->return_due_date)
                                                        <div class="text-xs font-semibold text-rose-600 flex items-center gap-1">
                                                            <i class="bi bi-calendar-event"></i>
                                                            <span>Return Due: {{ $order->return_due_date->format('d M, Y') }}</span>
                                                        </div>
                                                        @endif

                                                        {{-- Fallback dash if BOTH fields are empty --}}
                                                        @if(!$order->return_note && !$order->return_due_date)
