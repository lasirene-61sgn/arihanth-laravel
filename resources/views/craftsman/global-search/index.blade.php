...
@section('title', 'Global Search | Craftsman')

@section('content')
<div class="max-w-5xl mx-auto py-8">
    <div class="mb-8 text-center">
        <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-3">Global Search</h2>
        <p class="text-gray-600 dark:text-gray-400">Search across your assigned Work Orders, Repairs, Purchase Orders, Products, Stock Orders, and My Favorites.</p>
    </div>

    <!-- Search Form -->
    <div class="mb-10">
        <form action="{{ route('craftsman.global-search') }}" method="GET" class="relative" id="searchForm" onsubmit="return false;" enctype="multipart/form-data">
            <div class="relative flex items-center w-full h-16 rounded-2xl bg-white dark:bg-slate-900 shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-emerald-600 transition-all">
                ...
                <input 
                    ...
                    placeholder="Search custom design names, order numbers, products, repairs..." 
                    autocomplete="off"
                    autofocus 
                />

                <!-- Image Upload Trigger -->
                <label for="imageUploadInput" class="h-full grid place-items-center w-14 text-gray-400 hover:text-emerald-600 transition-colors cursor-pointer" title="Search by image">
                    <i class="bi bi-camera text-xl"></i>
                </label>
                <input type="file" id="imageUploadInput" class="hidden" accept="image/*" name="image_search" />

                <!-- Clear Button -->
                <a href="{{ route('craftsman.global-search') }}" id="clearBtn" class="h-full grid place-items-center w-14 text-gray-400 hover:text-emerald-600 transition-colors mr-2 {{ request('search') ? '' : 'hidden' }}">
            ...
            <div id="imagePreviewContainer" class="hidden mt-4 flex items-center justify-between bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm max-w-md mx-auto animate-fade-in-up">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-emerald-600 font-medium">Matching your designs & favorites</p>
                    </div>
                </div>
        ...
        @if(request()->has('search') && !empty($query))
            @if(empty($results))
                <div class="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 animate-fade-in-up">
                    ...
                    <p class="text-gray-500 dark:text-gray-400">We couldn't find anything matching "<span class="font-medium text-emerald-600">{{ $query }}</span>".</p>
                </div>
            @else
                <div class="mb-4 flex items-center justify-between animate-fade-in-up">
                    ...
                    <span class="text-sm font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full">
                        ...
                        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-gray-100 dark:border-slate-800 overflow-hidden animate-fade-in-up">
                            <div class="bg-gray-50 dark:bg-slate-800/50 px-5 py-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                                <h4 class="font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                                    @if($isFav)
                                        <i class="bi bi-heart-fill text-pink-600 text-sm"></i>
                                    @else
                                        <div class="w-2 h-2 rounded-full bg-emerald-600"></div>
                                    @endif
                                    {{ $category }}
                                </h4>
                                <span class="{{ $isFav ? 'bg-pink-600' : 'bg-emerald-600' }} text-white text-xs font-bold px-2.5 py-1 rounded-full">{{ $data['count'] }}</span>
                            </div>
                            <ul class="divide-y divide-gray-50 dark:divide-slate-800/50 max-h-72 overflow-y-auto custom-scrollbar">
                                @foreach($data['items'] as $item)
                                    ...
                                    <li>
                                        <button type="button" 
                                            class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-emerald-50/50 dark:hover:bg-slate-800 transition-colors group"
                                            data-url="{{ $item['url'] }}"
                                            ...
                                            <div class="flex items-center gap-3.5 min-w-0">
                                                ...
                                                @else
                                                    <div class="w-12 h-12 rounded-xl {{ $isFav ? 'bg-pink-50 text-pink-600' : 'bg-emerald-50 text-emerald-600' }} flex items-center justify-center shrink-0">
                                                        <i class="bi {{ $isFav ? 'bi-heart-fill' : 'bi-link-45deg' }} text-lg"></i>
                                                    </div>
                                                @endif
                                                <div class="min-w-0">
                                                    <span class="text-gray-800 dark:text-gray-200 font-semibold group-hover:text-emerald-700 transition-colors block truncate">{!! $displayHtml !!}</span>
                                                    <p class="text-xs text-gray-400 dark:text-gray-500 truncate mt-0.5">{!! $detailsHtml !!}</p>
                                                </div>
                                            </div>
                                            <i class="bi bi-chevron-right text-gray-300 group-hover:text-emerald-600 transition-colors text-sm mt-3 ml-2 shrink-0"></i>
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden transform scale-95 transition-transform duration-300" id="detailsModalContent">
            <div class="p-6">
                ...
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                    ...
                    <a id="modalLink" href="#" class="px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors shadow-md flex items-center">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderResults(query, results) {
            ...
            categories.forEach((category, index) => {
                ...
                const badgeBg = isFav ? 'bg-pink-600' : 'bg-emerald-600';
                ...
                html += `
                    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-gray-100 dark:border-slate-800 overflow-hidden animate-fade-in-up" style="animation-delay: ${animationDelay}s; opacity: 0; animation-fill-mode: forwards;">
                        <div class="bg-gray-50 dark:bg-slate-800/50 px-5 py-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                            ...
                            <span class="${badgeBg} text-white text-xs font-bold px-2.5 py-1 rounded-full">${data.count}</span>
                        </div>
                        <ul class="divide-y divide-gray-50 dark:divide-slate-800/50 max-h-72 overflow-y-auto custom-scrollbar">
                `;
                ...
                data.items.forEach(item => {
                    ...
                    const fallbackIcon = isFav
                        ? `<div class="w-12 h-12 rounded-xl bg-pink-50 text-pink-600 flex items-center justify-center shrink-0"><i class="bi bi-heart-fill text-lg"></i></div>`
                        : `<div class="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0"><i class="bi bi-link-45deg text-lg"></i></div>`;
                    ...
                    html += `
                        <li>
                            <button type="button" class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-emerald-50/50 dark:hover:bg-slate-800 transition-colors group"
                                data-url="${item.url}"
                                data-category="${category}"
        ...
        const performSearch = () => {
            ...
            searchIconContainer.innerHTML = `<div class="spinner-border text-emerald-600 w-5 h-5" role="status"></div>`;

            clearTimeout(debounceTimer);
            debounceTimer = setTimeout(() => {
                const formData = new FormData();
                let url = `{{ route('craftsman.global-search') }}`;
                let options = {
                    headers: {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (category) {
                ...
                if (category === 'My Favorites' || category === 'Favorites') {
                    categoryBadge.className = "text-[11px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full mb-1.5 inline-block bg-pink-50 text-pink-700 border border-pink-200";
                    modalBtnText.textContent = "View Design Details";
                } else {
                    categoryBadge.className = "text-[11px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full mb-1.5 inline-block bg-emerald-50 text-emerald-700 border border-emerald-200";
