...
<div class="max-w-5xl mx-auto py-8">
    <div class="mb-8 text-center">
        ...
        <p class="text-gray-600 dark:text-gray-400">Search across your assigned Work Orders, Repairs, Purchase Orders, Products, and Stock Orders.</p>
    </div>
            ...
            <div id="imagePreviewContainer" class="hidden mt-4 flex items-center justify-between bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm max-w-md mx-auto animate-fade-in-up">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-emerald-600">Visual similarity matching</p>
                    </div>
                </div>
                    ...
                    @foreach($results as $category => $data)
                        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-gray-100 dark:border-slate-800 overflow-hidden animate-fade-in-up">
                            <div class="bg-gray-50 dark:bg-slate-800/50 px-5 py-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                                ...
                                <span class="bg-emerald-600 text-white text-xs font-bold px-2 py-1 rounded-full">{{ $data['count'] }}</span>
                            </div>
                            <ul class="divide-y divide-gray-50 dark:divide-slate-800/50 max-h-72 overflow-y-auto custom-scrollbar">
                                @foreach($data['items'] as $item)
                                    <li>
                                        <button type="button" 
                                            class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-emerald-600/5 dark:hover:bg-slate-800 transition-colors group"
                                            data-url="{{ $item['url'] }}"
                                            ...
                                            <div class="flex items-center gap-3.5 min-w-0">
                                                ...
                                                @else
                                                    <div class="w-12 h-12 rounded-xl bg-emerald-600/10 flex items-center justify-center text-emerald-600 shrink-0">
                                                        <i class="bi bi-link-45deg text-lg"></i>
                                                    </div>
                                                @endif
                                                <div class="min-w-0">
                                                    <span class="text-gray-800 dark:text-gray-200 font-semibold group-hover:text-emerald-600 transition-colors block truncate">{{ $item['display'] }}</span>
            ...
            categories.forEach((category, index) => {
                ...
                data.items.forEach(item => {
                    ...
                    const imgHtml = item.image 
                        ? `<img src="${item.image}" alt="Preview" class="w-12 h-12 rounded-xl object-cover border border-gray-200 dark:border-slate-700 shrink-0" />`
                        : `<div class="w-12 h-12 rounded-xl bg-emerald-600/10 flex items-center justify-center text-emerald-600 shrink-0"><i class="bi bi-link-45deg text-lg"></i></div>`;

                    html += `
                        <li>
                            <button type="button" class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-emerald-600/5 dark:hover:bg-slate-800 transition-colors group"
                                data-url="${item.url}"
