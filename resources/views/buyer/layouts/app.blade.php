...
<body class="h-full font-sans antialiased text-slate-900 transition-colors duration-300" style="background-color:#fdfaff;">
    ...
    <div class="flex min-h-screen relative">
        ...
        @php
        $authUser = Auth::guard('buyer')->user() ?? Auth::guard('key_user')->user();
        @endphp
        ...
        <aside id="sidebar" class="sidebar-transition fixed inset-y-0 left-0 z-50 w-64 border-r border-purple-900 transform -translate-x-full lg:translate-x-0 lg:static lg:inset-0 flex flex-col" style="background: linear-gradient(180deg, #2d1b69 0%, #4a1d96 100%); box-shadow: 4px 0 20px rgba(0,0,0,0.3);">
            ...
            <div class="flex-1 overflow-y-auto py-4 px-4 space-y-1">
                <nav class="space-y-1">
                    {{-- Dashboard: Accessible to all authenticated accounts --}}
                    ...
                    {{-- Products --}}
                    @if($authUser && $authUser->hasPermission('product'))
                    ...
                    {{-- Work Orders --}}
                    @if($authUser && $authUser->hasPermission('work_order'))
                    ...
                    {{-- Live Stock Order --}}
                    @if($authUser && $authUser->hasPermission('stock_order'))
                    ...
                    {{-- Repairs --}}
                    @if($authUser && $authUser->hasPermission('repairs'))
                    ...
                    {{-- Designs --}}
                    @if($authUser && $authUser->hasPermission('design'))
                    ...
                    {{-- Favorites --}}
                    @if($authUser && $authUser->hasPermission('favorites'))
                    ...
                    {{-- Catalogues --}}
                    @if($authUser && $authUser->hasPermission('catalogue'))
                    ...
                    {{-- Key Users --}}
                    @if($authUser && $authUser->hasPermission('key_user'))
                    ...
                    {{-- Finance --}}
                    @if($authUser && $authUser->hasPermission('finance'))
