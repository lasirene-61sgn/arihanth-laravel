        ...
        <div class="p-6 md:p-10">
            ...
            <div class="mt-12 pt-8 border-t border-slate-100">
                <div class="flex items-center gap-2 mb-8">
                    ...
                    <h3 class="text-sm font-bold text-slate-800 uppercase tracking-widest">Order Tracking Timeline</h3>
                </div>
                
                <div class="relative pl-6 border-l-2 border-slate-200 space-y-8 ml-2">
                    
                    <!-- Created -->
                    <div class="relative">
                        <span class="absolute w-3.5 h-3.5 bg-blue-500 rounded-full -left-[1.8rem] top-1 ring-4 ring-white"></span>
                        <h6 class="text-sm font-bold text-slate-800">Order Created</h6>
                        <p class="text-xs text-slate-500 mt-1 font-medium"><i class="bi bi-clock mr-1"></i> {{ $workOrder->created_at ? $workOrder->created_at->timezone('Asia/Kolkata')->format('d M Y h:i A') : 'N/A' }}</p>
                    </div>

                    <!-- Due Date -->
                    <div class="relative">
                        <span class="absolute w-3.5 h-3.5 bg-amber-500 rounded-full -left-[1.8rem] top-1 ring-4 ring-white"></span>
                        <h6 class="text-sm font-bold text-slate-800">Expected Delivery</h6>
                        <p class="text-xs text-slate-500 mt-1 font-medium"><i class="bi bi-calendar-event mr-1"></i> {{ $workOrder->due_date ? $workOrder->due_date->format('d M Y') : 'Not Set' }}</p>
                    </div>

                    <!-- Completed -->
                    @if($workOrder->status === 'completed')
                    <div class="relative">
                        <span class="absolute w-3.5 h-3.5 bg-emerald-500 rounded-full -left-[1.8rem] top-1 ring-4 ring-white"></span>
                        <h6 class="text-sm font-bold text-emerald-700">Order Completed</h6>
                        <p class="text-xs text-slate-500 mt-1 font-medium"><i class="bi bi-check-circle-fill text-emerald-500 mr-1"></i> {{ $workOrder->updated_at ? $workOrder->updated_at->timezone('Asia/Kolkata')->format('d M Y h:i A') : 'N/A' }}</p>
                    </div>
                    @endif
