...
<div class="tw-max-w-full tw-mx-auto">
    ...
    <div class="tw-flex tw-items-center tw-gap-6">
        ...
        <div class="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
            <button class="tw-px-3 tw-py-1.5 tw-rounded-xl tw-bg-gray-50 dark:tw-bg-gray-700 tw-text-gray-600 dark:tw-text-gray-200 tw-text-[10px] tw-font-bold hover:tw-bg-gray-100 tw-transition-all" onclick="toggleSection('searchSection')">
                <i class="bi bi-search tw-mr-1"></i> SEARCH
            </button>
            <button class="tw-px-3 tw-py-1.5 tw-rounded-xl tw-bg-gray-50 dark:tw-bg-gray-700 tw-text-gray-600 dark:tw-text-gray-200 tw-text-[10px] tw-font-bold hover:tw-bg-gray-100 tw-transition-all" onclick="toggleSection('filterSection')">
                <i class="bi bi-funnel tw-mr-1"></i> FILTER
            </button>
            <button class="tw-px-3 tw-py-1.5 tw-text-[9px] tw-text-gray-400 hover:tw-text-brand tw-font-black tw-uppercase tw-tracking-widest" onclick="resetAll()">
                RESET VIEW
            </button>
        </div>
    </div>
            ...
            <form action="{{ route('super-admin.product.bulk-upload') }}" method="POST" enctype="multipart/form-data" class="tw-flex tw-flex-col lg:tw-flex-row tw-items-center tw-gap-6">
                ...
                <div class="tw-flex-1 tw-w-full">
                    <div class="tw-flex tw-items-center tw-gap-3">
                        <div class="tw-flex-1">
                            <input type="file" name="zip_file" class="form-control tw-rounded-xl tw-border-gray-200 tw-text-sm" accept=".zip" required>
                        </div>
    ...
    <div id="searchSection" class="tw-hidden tw-mb-4 tw-p-4 tw-bg-white dark:tw-bg-gray-800 tw-rounded-xl tw-border tw-border-gray-100 tw-shadow-sm">
        <form method="GET" action="{{ route('super-admin.product.index') }}" class="tw-flex tw-gap-2">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name or code..." class="form-control tw-rounded-xl tw-text-sm">
            <button type="submit" class="tw-bg-brand tw-text-white tw-px-6 tw-rounded-xl tw-text-xs tw-font-bold">APPLY</button>
        </form>
    </div>

    <div id="filterSection" class="tw-hidden tw-mb-4 tw-p-4 tw-bg-white dark:tw-bg-gray-800 tw-rounded-xl tw-border tw-border-gray-100 tw-shadow-sm">
        <form method="GET" action="{{ route('super-admin.product.index') }}" class="tw-flex tw-gap-2">
            <select name="category_filter" class="form-select tw-rounded-xl tw-text-sm">
                <option value="">All Categories</option>
                @foreach($categories as $cat)
                    <option value="{{ $cat->id }}" {{ request('category_filter') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                @endforeach
            </select>
            <button type="submit" class="tw-bg-brand tw-text-white tw-px-6 tw-rounded-xl tw-text-xs tw-font-bold">FILTER</button>
        </form>
    </div>
                ...
                <tbody class="tw-divide-y tw-divide-gray-50 dark:tw-divide-gray-700">
                    ...
                    <tr class="hover:tw-bg-gray-50/80 dark:hover:tw-bg-gray-900/40 tw-transition-colors">
                        ...
                        <td class="tw-p-4">
                            ...
                            @else
                                <div class="tw-w-10 tw-h-10 tw-bg-gray-50 tw-rounded-lg tw-flex tw-items-center tw-justify-center tw-text-[10px] tw-text-gray-300">NA</div>
                            @endif
                        </td>
                        <td class="tw-p-4 tw-text-xs tw-font-bold tw-text-brand">{{ $product->product_code }}</td>
                        ...
                        <td class="tw-p-4 tw-text-[10px] tw-text-gray-400">{{ $product->created_at->format('d M, y') }}</td>
                        <td class="tw-p-4">
                            <div class="tw-flex tw-gap-1">
                                ...
                                <form action="{{ route('super-admin.product.destroy', $product) }}" method="POST" onsubmit="return confirm('Delete?');" class="tw-inline">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="9" class="tw-p-10 tw-text-center tw-text-gray-400 tw-text-xs">No products found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<script>
    function toggleSection(id) {
        ...
        sections.forEach(s => {
            if(s === id) document.getElementById(s).classList.toggle('tw-hidden');
            else document.getElementById(s).classList.add('tw-hidden');
        });
    }
    function resetAll() { window.location.href = "{{ route('super-admin.product.index') }}"; }
