...
@section('title', 'Global Search | Key User')

@section('content')
<div class="max-w-5xl mx-auto py-8">
    <div class="mb-8 text-center">
        <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">Global Search</h2>
        <p class="text-gray-600 dark:text-gray-400">Search across all modules: Work Orders, Products, Catalogue, Designs, and Buyers.</p>
    </div>

    <!-- Search Form -->
    <div class="mb-10">
        <form action="{{ route('key-user.global-search') }}" method="GET" class="relative" id="searchForm" onsubmit="return false;" enctype="multipart/form-data">
            <div class="relative flex items-center w-full h-16 rounded-2xl bg-white dark:bg-slate-900 shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-amber-600 transition-all">
                ...
                <label for="imageUploadInput" class="h-full grid place-items-center w-14 text-gray-400 hover:text-amber-600 transition-colors cursor-pointer" title="Search by image">
                    <i class="bi bi-camera text-xl"></i>
                </label>
                <input type="file" id="imageUploadInput" class="hidden" accept="image/*" name="image_search" />

                <!-- Clear Button -->
                <a href="{{ route('key-user.global-search') }}" id="clearBtn" class="h-full grid place-items-center w-14 text-gray-400 hover:text-amber-600 transition-colors mr-2 {{ request('search') ? '' : 'hidden' }}">
            ...
            <div id="imagePreviewContainer" class="hidden mt-4 flex items-center justify-between bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm max-w-md mx-auto animate-fade-in-up">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-amber-600">Visual similarity matching</p>
                    </div>
                </div>
        ...
        @if(request()->has('search') && !empty($query))
            @if(empty($results))
                <div class="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 animate-fade-in-up">
                    ...
                    <p class="text-gray-500 dark:text-gray-400">We couldn't find anything matching "<span class="font-medium text-amber-600">{{ $query }}</span>".</p>
                </div>
            @else
                    ...
                    @foreach($results as $category => $data)
                        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-gray-100 dark:border-slate-800 overflow-hidden animate-fade-in-up">
                            <div class="bg-gray-50 dark:bg-slate-800/50 px-5 py-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                                ...
                                <span class="bg-amber-600 text-white text-xs font-bold px-2 py-1 rounded-full">{{ $data['count'] }}</span>
                            </div>
                            <ul class="divide-y divide-gray-50 dark:divide-slate-800/50 max-h-72 overflow-y-auto custom-scrollbar">
                                @foreach($data['items'] as $item)
                                    <li>
                                        <button type="button" 
                                            class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-amber-600/5 dark:hover:bg-slate-800 transition-colors group"
                                            data-url="{{ $item['url'] }}"
                                            ...
                                            <div class="flex items-center gap-3.5 min-w-0">
                                                ...
                                                @else
                                                    <div class="w-12 h-12 rounded-xl bg-amber-600/10 flex items-center justify-center text-amber-600 shrink-0">
                                                        <i class="bi bi-link-45deg text-lg"></i>
                                                    </div>
                                                @endif
                                                <div class="min-w-0">
                                                    <span class="text-gray-800 dark:text-gray-200 font-semibold group-hover:text-amber-600 transition-colors block truncate">{{ $item['display'] }}</span>
                                                    <p class="text-xs text-gray-400 dark:text-gray-500 truncate mt-0.5">{{ $item['details'] ?? 'No additional details available.' }}</p>
                                                </div>
                                            </div>
                                            <i class="bi bi-chevron-right text-gray-300 group-hover:text-amber-600 transition-colors text-sm mt-3 ml-2 shrink-0"></i>
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden transform scale-95 transition-transform duration-300" id="detailsModalContent">
            <div class="p-6">
                ...
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                    ...
                    <a id="modalLink" href="#" class="px-4 py-2 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-xl transition-colors shadow-md flex items-center">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            categories.forEach((category, index) => {
                ...
                data.items.forEach(item => {
                    ...
                    const imgHtml = item.image 
                        ? `<img src="${item.image}" alt="Preview" class="w-12 h-12 rounded-xl object-cover border border-gray-200 dark:border-slate-700 shrink-0" />`
                        : `<div class="w-12 h-12 rounded-xl bg-amber-600/10 flex items-center justify-center text-amber-600 shrink-0"><i class="bi bi-link-45deg text-lg"></i></div>`;

                    html += `
                        <li>
                            <button type="button" class="search-result-btn w-full text-left flex items-start justify-between p-4 hover:bg-amber-600/5 dark:hover:bg-slate-800 transition-colors group"
                                data-url="${item.url}"
        ...
        const performSearch = () => {
            ...
            searchIconContainer.innerHTML = `<div class="spinner-border text-amber-600 w-5 h-5" role="status"></div>`;

            clearTimeout(debounceTimer);
            debounceTimer = setTimeout(() => {
                const formData = new FormData();
                let url = `{{ route('key-user.global-search') }}`;
                let options = {
                    headers: {
