...
class StockOrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('super-admin.stock-order.index', compact('orders', 'craftsmen', 'counts', 'activeTab'));
    }
    ...
    public function create()
    {
        ...
        return view('super-admin.stock-order.create', compact('buyers', 'categories'));
    }
    ...
    public function show(StockOrder $stockOrder)
    {
        ...
        return view('super-admin.stock-order.show', compact('stockOrder', 'craftsmen'));
    }
    ...
    public function edit(StockOrder $stockOrder)
    {
        ...
        $buyers = Buyer::where('is_frozen', false)->get();
        $categories = ProductCategory::all();
        return view('super-admin.stock-order.edit', compact('stockOrder', 'buyers', 'categories'));
    }
    ...
    public function update(Request $request, StockOrder $stockOrder)
    {
        ...
        DB::transaction(function () use ($request, $stockOrder) {
            ...
            // Alternatively, sync by ID if items have IDs
        ...
        return redirect()->route('super-admin.stock-order.index')->with('success', 'Stock order updated successfully!');
    }
    ...
    public function allocate(Request $request, StockOrder $stockOrder)
    {
        ...
        // Notify Craftsman
        if ($stockOrder->craftsman && method_exists($stockOrder->craftsman, 'notify')) {
    ...
    public function allocateItems(Request $request, StockOrder $stockOrder)
    {
        ...
        // Notify Craftsman
        $craftsman = \App\Models\Craftman::find($request->craftsman_id);
        if ($craftsman && method_exists($craftsman, 'notify')) {
            $craftsman->notify(new StockOrderAllocated($stockOrder));
        }
    ...
    public function updateStatus(Request $request, StockOrder $stockOrder)
    {
        ...
        if ($request->status === 'Completed' && $stockOrder->buyer && method_exists($stockOrder->buyer, 'notify')) {
    ...
    public function updateItemStatus(Request $request, StockOrder $stockOrder, $itemId)
    {
        ...
        $item->update([
            ...
            'rejection_reason' => null // Clear rejection reason if status is changed
        ]);
        ...
        if ($stockOrder->items()->where('status', '!=', 'Completed')->count() === 0) {
            ...
            // Notify Buyer on order completion
    ...
    {
        $stockOrder->delete();
        return redirect()->route('super-admin.stock-order.index')->with('success', 'Order deleted successfully.');
    }
    ...
    public function bulkAllocate(Request $request)
    {
        ...
        foreach ($request->order_ids as $orderId) {
            ...
            if ($order && $craftsman && method_exists($craftsman, 'notify')) {
                $craftsman->notify(new StockOrderAllocated($order));
            }
        }
    ...
    public function bulkComplete(Request $request)
    {
        ...
        foreach ($request->order_ids as $orderId) {
            $order = StockOrder::find($orderId);
            if ($order && $order->buyer && method_exists($order->buyer, 'notify')) {
                $order->buyer->notify(new StockOrderCompleted($order));
            }
        }
