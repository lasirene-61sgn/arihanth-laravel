...
class WorkOrderController extends Controller
{
    ...
    /**
     * Helper method to get the authenticated user's BP code
     * Works for both key users and buyers
     */
    private function getAuthenticatedUserBpCode()
    {
        $keyUser = auth()->guard('key_user')->user();
        if ($keyUser) {
            return $keyUser->bp_code;
        }

        $buyer = auth()->guard('buyer')->user();
        if ($buyer) {
            return $buyer->bp_code;
        }
    ...
    public function index(Request $request)
    {
        ...
        // Get the current user's BP code (works for both key users and buyers)
        $userBpCode = $this->getAuthenticatedUserBpCode();

        if (!$userBpCode) {
            return redirect()->route('buyer.login.form')->with('error', 'Please log in to continue.');
        }

        // For buyers, show all work orders with their BP code regardless of status
        $allOrdersQuery = WorkOrder::with(['product.images'])->where('bp_code', $userBpCode);
        ...
        $allocatedOrders = $allOrdersQuery->clone()->where('status', 'allocated')->where('craftsman_status', '!=', 'in_process')->paginate($perPage, ['*'], 'allocated_orders_page');
        $forApprovalOrders = $allOrdersQuery->clone()->where('status', 'for_approval')->paginate($perPage, ['*'], 'for_approval_orders_page');
        $completedOrders = $allOrdersQuery->clone()->where('status', 'completed')->paginate($perPage, ['*'], 'completed_orders_page');
        $inProcessOrders = $allOrdersQuery->clone()->where('craftsman_status', 'in_process')->paginate($perPage, ['*'], 'in_process_orders_page');
        $overdueOrders = $allOrdersQuery->clone()->where('craftsman_status', 'Overdue')->paginate($perPage, ['*'], 'overdue_orders_page');
        $rejectedOrders = $allOrdersQuery->clone()->where('craftsman_status', 'rejected')->paginate($perPage, ['*'], 'rejected_orders_page');
        $allOrders = $allOrdersQuery->clone()->paginate($perPage, ['*'], 'all_orders_page');

        // Get categories and subcategories for filters
        $categories = ProductCategory::all();
        $subcategories = ProductSubcategory::all();

        return view('buyer.work-order.index', compact(
            'newOrders',
            'allocatedOrders',
            'forApprovalOrders',
            'completedOrders',
    ...
    private function loadWorkOrdersAjax(Request $request)
    {
        ...
        $query = WorkOrder::with(['product.images'])->where('bp_code', $userBpCode);
        ...
        switch ($tab) {
            ...
            case 'allocated-orders':
                $workOrders = $query->where('status', 'allocated')->where('craftsman_status', '!=', 'in_process')->paginate($perPage);
                break;
            case 'for-approval-orders':
                $workOrders = $query->where('status', 'for_approval')->paginate($perPage);
                break;
            case 'in-process-orders':
                $workOrders = $query->where('craftsman_status', 'in_process')->paginate($perPage);
                break;
            case 'overdue-orders':
                $workOrders = $query->where('craftsman_status', 'Overdue')->paginate($perPage);
                break;
            case 'completed-orders':
                $workOrders = $query->where('status', 'completed')->paginate($perPage);
                break;
        ...
        return response()->json([
            'html' => view('buyer.work-order.partials.orders-table', compact('workOrders'))->render(),
    ...
    public function create()
    {
        ...
        $buyers = Buyer::where('bp_code', $userBpCode)->get();
        $categories = ProductCategory::orderBy('name')->get();

        return view('buyer.work-order.create', compact('buyers', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Get the authenticated user (either key user or buyer) to track who created the work order
        $keyUser = auth()->guard('key_user')->user();
        $buyer = auth()->guard('buyer')->user();
        $userId = $keyUser ? $keyUser->id : ($buyer ? $buyer->id : null);

        // If customer name wasn't provided, get it from the buyer
        $customerName = $request->customer_name;
        if (empty($customerName)) {
            $buyerModel = Buyer::where('bp_code', $userBpCode)->first();
            $customerName = $buyerModel ? $buyerModel->business_name : '';
        }
        ...
        $workOrder = WorkOrder::create([
            ...
            'bp_code' => $userBpCode, // Auto-set from user's BP
            'customer_name' => $customerName, // Auto-set or from request
            ...
            // Note: No craftsman_due_date and narration_craftsman for key user
        ...
        return redirect()->route('buyer.work-order.index', ['tab' => 'new-orders'])
    ...
    {
        return view('buyer.work-order.print', compact('workOrder'));
    }
    ...
    public function edit(WorkOrder $workOrder)
    {
        ...
        return view('buyer.work-order.edit', compact('workOrder', 'categories', 'buyers', 'products'));
    }
    ...
    public function update(Request $request, WorkOrder $workOrder)
    {
        ...
        return redirect()->route('buyer.work-order.index')->with('success', 'Work Order updated successfully!');
    }
    ...
    /**
     * Show the bulk allocation form.
     */
    public function bulkAllocateForm(Request $request)
    {
        // Key users don't have bulk allocation functionality
        return redirect()->back()->with('error', 'Bulk allocation is not available for key users.');
    }
    /**
     * Handle bulk printing of selected work orders.
     */
    public function bulkPrint(Request $request)
    {
        $ids = $request->input('selected_orders', []);
        $workOrders = WorkOrder::whereIn('id', $ids)
            ->with(['product.images', 'productCategory', 'subcategoryRelation', 'buyer', 'craftsman'])
            ->get();

        return view('buyer.work-order.bulk-print', compact('workOrders'));
    }
}
