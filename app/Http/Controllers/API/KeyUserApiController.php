...
class KeyUserApiController extends Controller
{
    // =========================================================================
    // AUTHENTICATION
    // =========================================================================
    ...
    public function login(Request $request)
    {
        ...
        $keyUser = KeyUser::where('user_code', $request->user_code)->first();

        if (!$keyUser || !Hash::check($request->password, $keyUser->password)) {
        ...
        $token = $keyUser->createToken('KeyUserAuthToken')->plainTextToken;
    ...
    {
        return response()->json(['key_user' => $request->user()]);
    }
    ...
    public function getWorkOrders(Request $request)
    {
        ...
        if (!$keyUser->hasPermission('work_order')) return response()->json(['message' => 'Forbidden'], 403);

        $perPage = $request->get('per_page', 10);

        $query = WorkOrder::with(['product.images'])
            ->where('bp_code', $keyUser->bp_code); // Key Users see all work orders for their Buyer
    ...
    public function storeWorkOrder(Request $request)
    {
        ...
        $workOrder = WorkOrder::create([
            ...
            'customer_name' => $request->customer_name ?? $keyUser->buyer->business_name,
            ...
            'status' => 'new',
            'created_by' => $keyUser->id,
            'creator_type' => 'key_user', // Important difference
            'creator_user_code' => $keyUser->user_code,
        ]);
    ...
    public function updateWorkOrder(Request $request, $id)
    {
        ...
        $workOrder = WorkOrder::where('bp_code', $keyUser->bp_code)->find($id);
    ...
    public function deleteWorkOrder(Request $request, $id)
    {
        ...
        // Optional: Check if key user can delete works created by others (web controller suggests ownership check isn't strictly enforced for delete within same BP code usually, but good to be aware. Sticking to BP code check for now as per Buyer implementation).
    ...
    {
        $keyUser = $request->user();
        if (!$keyUser->hasPermission('product')) return response()->json(['message' => 'Forbidden'], 403);
    ...
    public function storeProduct(Request $request)
    {
        ...
        $product = Product::create([
            ...
            'created_by' => $keyUser->id, // Tracking Creator (Key User ID)
        ]);
    ...
    public function updateProduct(Request $request, $id)
    {
        ...
        $product = Product::where('bp_code', $keyUser->bp_code)->find($id);
        if (!$product) return response()->json(['message' => 'Product not found'], 404);
    ...
    // =========================================================================
    // DESIGNS (Global - Read Only for Key User)
    // =========================================================================
    ...
    {
        // KeyUser guard doesn't have isDesignLocked logic on the model usually, checking Buyer logic:
        // KeyUsers access designs via their Buyer's context or globally?
        // Web controller for KeyUser Design uses standard Product find.
        // Assuming READ ONLY is allowed for accepted global designs.
    ...
    {
        $keyUser = $request->user();
        // Assuming Key Users generally have permission to manage End Users if they have dashboard access, or specific permission?
        // Web controller didn't explicitly check a permission for 'users' index, just bp_code linkage.

        $query = User::where('bp_code', $keyUser->bp_code);
    ...
    {
        $keyUser = $request->user();
        if ($keyUser->is_frozen) return response()->json(['message' => 'Account frozen'], 403);
    ...
    {
        $keyUser = $request->user();
        $user = User::where('bp_code', $keyUser->bp_code)->find($id);
