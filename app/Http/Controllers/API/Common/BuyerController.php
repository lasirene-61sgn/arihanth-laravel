...
class BuyerController extends Controller
{
    ...
    /**
     * Display a listing of buyers (Admin/SuperAdmin)
     */
    ...
    public function getProfile(Request $request)
    {
        $user = $request->user();
        if (!($user instanceof Buyer)) {
            return response()->json(['message' => 'Unauthorized or not a Buyer'], 401);
        }

        $user->load(['aadharDetails', 'panDetails', 'bankDetails']);
        return response()->json(['buyer' => $user]);
    }

    /**
     * Display specific Buyer (Admin or Self)
     */
    public function show(Request $request, $id)
    {
        ...
        $query = Buyer::with(['aadharDetails', 'panDetails', 'bankDetails']);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($scope === 'self' && $buyer->kyc_status === 'approved') {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'bp_code' => 'sometimes|string|unique:buyers,bp_code,' . $buyer->id,
        ...
        foreach ($files as $file) {
            if ($request->hasFile($file)) {
                if ($buyer->$file) Storage::disk('public')->delete($buyer->$file);
        ...
        // Related Details (Aadhar/PAN/Bank) - simplified update logic often used in this project
        ...
        return response()->json(['message' => 'Buyer updated successfully', 'buyer' => $buyer->load(['aadharDetails', 'panDetails', 'bankDetails'])]);
    }
    ...
    /**
     * Generate PDF for Buyers
     */
    public function generatePdf(Request $request)
    {
        $user = $request->user();
        if ($this->getScope($user) !== 'global' && !($user instanceof Buyer)) {
            return response()->json(['message' => 'Forbidden'], 403);
        }
        ...
        $buyers = $query->get();
        if ($buyers->isEmpty()) return response()->json(['message' => 'No buyers found'], 404);
        ...
        try {
            ...
            $filename = count($buyers) === 1 ? "Buyer_" . $buyers->first()->bp_code . ".pdf" : "Buyers_Report_" . now()->format('Ymd_His') . ".pdf";

            return response($dompdf->output())
                ->header('Content-Type', 'application/pdf')
                ->header('Content-Disposition', 'attachment; filename="' . $filename . '"');
        } catch (\Exception $e) {
            Log::error('Buyer PDF Error: ' . $e->getMessage());
